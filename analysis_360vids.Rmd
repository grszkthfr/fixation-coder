...
| control_x         | x coordinate of *active* validation_point             |
...
if (run_python) {
    ...
    use_virtualenv("360vids")

    py_run_file("get_drift.py")

    # only read from files when results were exported
    read_files <- ifelse(!py$EXPORT_RESULTS, FALSE, TRUE)
...
if (read_files) {
    ...
    print("reading .txt-files in...")

    validation_log <- 

    # get paths of all .csv-files in all subdirectories
    dir_ls("log_validation", recursive = TRUE, glob = "*.txt") %>%
    ...
    print("doing reticulate magic...")
    validation_log <- py$validation_log

}

```

```{r tidy validation log}
...
validation_summary <- 
    ...
    arrange(subject_id, session_id, validation_id)

```

## summary

```{r merge validation data & summary}
...
if (print_plots == TRUE) {
    ...
    walk2(
        ...
        # for each subject
        .x = validation_plot$subject_id,
    
        # each plot
        .y = validation_plot$plot,
...
df_video %>% 
    group_by(subject_id, video_id, session_id) %>% 
    iwalk
